...
def getCFactor():
    ...
    file = open(defeatureFile, "r")
    origLines = open(defeatureFile).readlines()
    lines = open(defeatureFile).readlines()
    ...
    for index, line in enumerate(file):
        ...
        if keyword in line:
            ...
            endOfBlock = findEndOfBlock(lineNumber)
            startOfBlock = findStartOfBlock(endOfBlock)
            if headerKeyword in lines[startOfBlock + 1]: #Changed to be one past startOfBlock to be more robust (instead of prevLine)
                indexOfVal = index
                break
    ...
    while indexOfVal < endOfBlock:
        if valueKeyword in lines[indexOfVal]:
            realValueLine = origLines[indexOfVal] # Updated to pull from original casing lines, and not lowercase-lines
            ...
            #UPDATE: Only capturing string now to prevent any future decimal cutoffs from Python-float:
            cFactorVal = realValueString
            #UPDATE: Remove whitespace from extraction so only real value is returned:
            cFactorVal = cFactorVal.replace(" ", "")
            break
        indexOfVal = indexOfVal + 1

    file.close()

    # Return the found CFactor value as a string
    return cFactorVal
#-----------------------------------------------------------------------------------------------------------------------
...
def getPressAngle():
    ...
    while indexOfVal < endOfBlock:
        if valueKeyword in lines[indexOfVal]:
            ...
            realValueString = realValueLine[snipStartIndex:snipEndIndex]
            #UPDATE: Only capturing string now to prevent any future decimal cutoffs from Python-float:
            pressAngleVal = realValueString
            #UPDATE: Remove whitespace from extraction so only real value is returned:
            pressAngleVal = pressAngleVal.replace(" ", "")
            break
        indexOfVal = indexOfVal + 1

    file.close()

    # Return the found Pressure Angle value as a string
...
def getHelixAngle():
    ...
    # Loop through by index to find the exact block where the real_value for rack helix angle is located; save a ref. index
    for index, line in enumerate(file):
        line = line.lower()
        if keyword in line:
            lineNumber = index
    ...
    # Use previously saved index of rack helix angle block and search for subsequent real_value line until end of helix angle block
    while indexOfVal < endOfBlock:
        if valueKeyword in lines[indexOfVal]:
            realValueLine = origLines[indexOfVal] # Updated to pull from original casing lines, and not lowercase-lines
            snipStartIndex = (realValueLine.find("=") + 1)
            ...
            realValueString = realValueLine[snipStartIndex:snipEndIndex]
            #UPDATE: Only capturing string now to prevent any future decimal cutoffs from Python-float:
            helixAngleVal = realValueString
            #UPDATE: Remove whitespace from extraction so only real value is returned:
            helixAngleVal = helixAngleVal.replace(" ", "")
            break
        indexOfVal = indexOfVal + 1

    file.close()

    # Return the found Helix Angle value as a string
...
def getRBSpringStiffness():
    ...
    while indexOfVal < endOfBlock:
        if valueKeyword in lines[indexOfVal]:
            ...
            realValueString = realValueLine[snipStartIndex:snipEndIndex]
            #UPDATE: Only capturing string now to prevent any future decimal cutoffs from Python-float:
            RBSpringStiffVal = realValueString
            #UPDATE: Remove whitespace from extraction so only real value is returned:
            RBSpringStiffVal = RBSpringStiffVal.replace(" ", "")
            break
        indexOfVal = indexOfVal + 1

    file.close()

    # Return the found spring stiffness value as a string
...
def getRBSpringPreload():
    ...
    while indexOfVal < endOfBlock:
        if valueKeyword in lines[indexOfVal]:
            ...
            realValueString = realValueLine[snipStartIndex:snipEndIndex]
            #UPDATE: Only capturing string now to prevent any future decimal cutoffs from Python-float:
            RBSpringPreloadVal = realValueString
            #UPDATE: Remove whitespace from extraction so only real value is returned:
            RBSpringPreloadVal = RBSpringPreloadVal.replace(" ", "")
            break
        indexOfVal = indexOfVal + 1

    file.close()

    # Return the found spring preload value as a string
...
def getNewRPGearSepEqu():
    newEqu = "2* PI * ABS(VARVAL(." + fileName + ".VAR_reps_meas_pinionTorque_Nmm) / " + (getRealValue("rpCFactor")) + " * TAN(" + (getRealValue("rpNormPressAng")) + "d) / COS(" + (getRealValue("rackHelixAng")) + "d))"
    return newEqu
#-----------------------------------------------------------------------------------------------------------------------
...
def changeRBSpringPreloadToVal():
    ...
    if hasKeyword("rackBrgSpr_preload", fileLines):
        ...
        RBPreloadRealValue = getRealValue("rackBrgSpr_preload")
...
def changeRBSpringStiffToVal():
    ...
    if hasKeyword("rackBrgSpr_stiff", fileLines):
        ...
        RBStiffnessRealValue = getRealValue("rackBrgSpr_stiff")
...
def findAndRemoveUnassociated():
    ...
    for index, line in enumerate(file):
        if ("variable_name" in fileLines[index]):
            lineNumber = index
            endOfBlock = findEndOfBlock(lineNumber)
            startOfBlock = findStartOfBlock(endOfBlock)
            if ("variable create" in fileLines[startOfBlock + 1]):
                # Extract that variable name for processing
                snipStartIndex = line.find("=") + 1
                snipEndIndex = line.find("&")
                fullVarName = line[snipStartIndex:snipEndIndex] # Using original-cased lines to preserve var name
    ...
    while i >= 0:
        ...
        for index, line in enumerate(file):
            if unassociatedVars[i] in line:
                ...
                if (("variable_name" in fileLines[index]) and (("variable modify" in fileLines[startOfBlock + 1]) or ("variable create" in fileLines[startOfBlock + 1]))):
                    pass
...
def okButtonPressed(root, defeatureEntry, backupEntry, messageWindowLabel, guiDir):
...
def startProcessWindow(oldRoot, guiDir):
    ...
    img = PhotoImage(file=(os.path.join(guiDir, r"NexteerBannerBTS.png")))
    imageLabel = Label(frame1, image=img)
    imageLabel.pack()

    secondRoot.iconbitmap((os.path.join(guiDir, r"NexteerIcon.ico")))
...
def mainGUI(guiDir):
    try:
        ...
        img3 = PhotoImage(file=(os.path.join(guiDir, r"NexteerBannerBckgrnd.png")))
        ...
        rootFrame.iconbitmap((os.path.join(guiDir, r"NexteerIcon.ico")))
        ...
        okButton = Button(frame2, text="OK", height=2, width=5, command=lambda: threading.Thread(target=okButtonPressed(rootFrame, defeatureEntry, backupEntry, messageWindowLabel, guiDir)).start())
...
if __name__ == "__main__":
    maxNumOfFolderChgAttempts = 0
    initialCWD = os.getcwd()
    wereGUIFilesNeverLocated = False
    try:
        #Get the current working file directory as to where this program is running from:
        cwd = os.getcwd()
        guiDir = cwd

        #Stores the root directory string, such as C:\ in C:\Sample\Dir
        os.chdir(r"/")
        rootDir = os.getcwd()

        #Go back to original working directory of program after storing root drive directory name:
        os.chdir(cwd)

        print("Root directory of this OS: " + rootDir)
        ...
        #WORKS:
        insideGUI = False
        while ((not(insideGUI)) and (os.getcwd() != rootDir)):
            try:
                print("Attempting: " + os.path.join(cwd, r"BTS3000_GUI_Files"))
                os.chdir(os.path.join(cwd, r"BTS3000_GUI_Files"))
                print("Moving GUI locator inside: " + os.getcwd())
                cwd = os.getcwd()
                insideGUI = True
            except:
                print("Didn't find the GUI folder, relocating...")
                try:
                    os.chdir(r"..")
                    cwd = os.getcwd()
                    insideGUI = False
                except:
                    print("ERR: Navigating up directory has failed. Aborting.")
                    wereGUIFilesNeverLocated = True
                    break
        #End of while

        #Do one last check in root (e.g. C:\) directory before giving up:
        if (os.getcwd() == rootDir):
            try:
                print("Attempting: " + (os.path.join(rootDir, r"BTS3000_GUI_Files")))
                os.chdir(os.path.join(rootDir, r"BTS3000_GUI_Files"))
                cwd = os.getcwd()
                insideGUI = True
            except:
                wereGUIFilesNeverLocated = True
                print("ERR: Whoops...GUI files folder was never found in program's parent directories.")

        if (insideGUI):
                guiDir = cwd
                print("The new guiDir is: " + guiDir)
        ...
        print("Linking GUI icon file to this dir: " + (os.path.join(guiDir, r"NexteerIcon.ico")))
        splashFrame.iconbitmap(os.path.join(guiDir, r"NexteerIcon.ico"))
        ...
        #img = PhotoImage(file=r"Drive:\[YOUR]\[GUI_Files]\[DIRECTORY]\[HERE]\NexteerBannerBTS.png")
        print("Linking GUI banner file to this dir: " + (os.path.join(guiDir, r"NexteerBannerBTS.png")))
        img = PhotoImage(file=(os.path.join(guiDir, r"NexteerBannerBTS.png")))
        ...
        print("Linking GUI splash screen file to this dir: " + (os.path.join(guiDir, r"NexteerBannerBTS.png")))
        loadImg = PhotoImage(file=(os.path.join(guiDir, r"Loading_Circle (1).png")))
        ...
        splashFrame.after(5000, lambda: mainGUI(guiDir))

        mainloop()
    except TclError:
        # Pop up an error window
        if (wereGUIFilesNeverLocated):
            tkinter.messagebox.showerror(title="GUI Error", message="Could not find GUI components' directory folder. "
                                                                "\nPlease make sure the GUI files folder, BTS3000_GUI_Files, "
                                                                "\nis either in the current working directory:\n" +
                                                                initialCWD + ", or is in parent folders ABOVE this directory."
                                                                "\n\nIf you would like to use a different DRIVE/specific directory"
                                                                "\nfor the GUI components folder, please edit the GUI file paths"
                                                                "\nin defeature kit V2 code, rebuild with pyinstaller,\n"
