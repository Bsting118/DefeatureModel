...
def findIndexOf(keyword, fileLines, fileLength):
    indexOf = 0
    while indexOf < fileLength:
...
def removeCylinder():
    ...
    for index, line in enumerate(file):
        if keyword in line:
            ...
            # Extra removal: if cylinder instance still exists, remove that block where it is as well
            if hasKeyword(cylName, lines):
                ...
                if "geometry attributes" in lines[startOfBlock + 1]:
...
def removeAllCylinders():
    ...
    while (hasKeyword("cylinder", currentFile)) == True:
        removeCylinder()
        currentFile = open(defeatureFile).readlines()
#-----------------------------------------------------------------------------------------------------------------------
...
def removeAllCylinderRefs():
    currentFile = open(defeatureFile).readlines()
    while (hasKeyword("cylRef", currentFile)) == True:
        removeCylinderRef()
        currentFile = open(defeatureFile).readlines()
#-----------------------------------------------------------------------------------------------------------------------
...
def removeAllMarkerModifies():
    currentFile = open(defeatureFile).readlines()
    while (hasKeyword("marker modify", currentFile)) == True:
        removeMarkerModify()
        currentFile = open(defeatureFile).readlines()
#-----------------------------------------------------------------------------------------------------------------------
...
def removeVarCreate(keyword):
    ...
    for index, line in enumerate(file):
        if keyword in line:  # Keyword will be the name of the variable whose create block you want deleted
            ...
            if ("variable create" in lines[startOfBlock + 1]) and ("variable_name" in lines[lineNumber]):
                removeBlock(startOfBlock, lines)
                break
    file.close()
#-----------------------------------------------------------------------------------------------------------------------
...
def removeVarModify(keyword):
    ...
    for index, line in enumerate(file):
        if keyword in line:
            ...
            endOfBlock = findEndOfBlock(lineNumber)
            startOfBlock = findStartOfBlock(endOfBlock)
            if ("variable modify" in lines[startOfBlock + 1]) and ("variable_name" in lines[lineNumber]):
                removeBlock(startOfBlock, lines)
                break
    file.close()
#-----------------------------------------------------------------------------------------------------------------------
...
def removeForceModify(keyword):
    ...
    for index, line in enumerate(file):
        if keyword in line:  # Keyword will be the name of the force whose modify block you want deleted
            ...
            startOfBlock = findStartOfBlock(endOfBlock)
            # The header is always one below the start which is top "!" (changed to include var name check)
            if ("force modify" in lines[startOfBlock + 1]) and ("force_name" in lines[lineNumber]):
                removeBlock(startOfBlock, lines)
                break
    file.close()
#-----------------------------------------------------------------------------------------------------------------------
...
def getOldRPGearSepEquV2():
    ...
    for index, line in enumerate(file):
        ...
        if "SFORCE_reps_RP_gearSep" in line:
...
def insertRPGearSepEqu():
    ...
    if hasKeyword("SFORCE_reps_RP_gearSep", fileLines):
        foundGearSep = True
        for line in file:
...
def changeVarToValAll(keyword):
    currentFile = open(defeatureFile).readlines()
    while (hasKeyword(keyword, currentFile)) == True:
...
def removeAllRepsCalcVar():
    currentFile = open(defeatureFile).readlines()
    while (hasKeyword("repsCalc", currentFile)) == True:
        removeRepsCalcVar()
        currentFile = open(defeatureFile).readlines()
#-----------------------------------------------------------------------------------------------------------------------
...
def findAndRemoveUnassociated():
    ...
    for index, line in enumerate(file):
        if ("variable_name" in line) and ("variable create" in fileLines[index - 1]):
...
def removeMarkerCreate(keyword):
    ...
    for index, line in enumerate(file):
        if keyword in line:  # Keyword will be the name of the marker whose create block you want deleted
            ...
            if ("marker create" in lines[startOfBlock + 1]) and ("marker_name" in lines[lineNumber]):
                removeBlock(startOfBlock, lines)
                break
...
def removeMarkerAttr(keyword):
    ...
    for index, line in enumerate(file):
        if keyword in line:  # Keyword will be the name of the variable whose create block you want deleted
            ...
            if ("marker attributes" in lines[startOfBlock + 1]) and ("marker_name" in lines[lineNumber]):
                removeBlock(startOfBlock, lines)
                break
...
def removeGroundFrameMarkers():
    ...
    while i < endOfSegment:
        ...
        if "marker create" in fileLines[i]:
...
def removeAllPSMAR():
    ...
    while hasKeyword("PSMAR", fileLines):
        removePSMAR()
        fileLines = open(defeatureFile).readlines()
#-----------------------------------------------------------------------------------------------------------------------
...
def removeAllBenchOC():
    ...
    while hasKeyword("benchOC", fileLines):
        removeBenchOCSpline()
        fileLines = open(defeatureFile).readlines()
#-----------------------------------------------------------------------------------------------------------------------
...
def isInDefaultGroundFrame(fileLines, startOfSegment):
    ...
    while (index < endOfFile) and (("!" not in fileLines[index]) or ("---" not in fileLines[index])):
        if ("default_coordinate_system" in fileLines[index]) and ("reps_ground_FRAME" in fileLines[index]):
...
def isPartSegment(fileLines, startOfSegment, segmentName):
    ...
    while (index < endOfFile) and (("!" not in fileLines[index]) or ("---" not in fileLines[index])):
        ...
        if "part create rigid_body name" in fileLines[index]:
            blockIndex = index
            ...
            while "!" not in fileLines[blockIndex]:
                ...
                if ("part_name" in fileLines[blockIndex]) and (segmentName in fileLines[blockIndex]):
                    isPart = True
                    break
...
def getStartOfSegment(segmentName, fileLines):
    ...
    while index < endOfFile:
        ...
        if (segmentName in fileLines[index]) and ("!" in fileLines[index]) and ("---" in fileLines[index]):
            startOfSegment = index
            break
        index = index + 1
...
def removeAllDataRefAxialSprNL():
    ...
    while hasKeyword("DATA_REF_reps_axialSprNL", fileLines):
...
def removeWIPBenchSprNL():
    ...
    for index, line in enumerate(file):
        if "DATA_WIP1_benchSprNL" in line:
            endOfBlock = findEndOfBlock(index)
            startOfBlock = findStartOfBlock(endOfBlock)
            # We check if it's a create block by looking one down from the starting block index which is a "!"
            if "data_element create" in fileLines[startOfBlock + 1]:
...
def removeAllWIPBenchSprNL():
    ...
    while hasKeyword("DATA_WIP1_benchSprNL", fileLines):
        removeWIPBenchSprNL()
        fileLines = open(defeatureFile).readlines()
#-----------------------------------------------------------------------------------------------------------------------
...
def removeAllDOBJAvg():
    ...
    while hasKeyword("DOBJ_avg", fileLines):
        removeDOBJAvg()
        fileLines = open(defeatureFile).readlines()
#-----------------------------------------------------------------------------------------------------------------------
...
def removeAllDOBJRms():
    ...
    while hasKeyword("DOBJ_rms", fileLines):
        removeDOBJRms()
        fileLines = open(defeatureFile).readlines()
#-----------------------------------------------------------------------------------------------------------------------
...
def startProcessWindow(oldRoot):
    ...
    # Alternatively, will look into using os.getcwd() and have GUI files at same level as code in Pyinstaller
